...
func TestRender(t *testing.T) {
	...
	tc := []struct {
	...
	}{
		{
			...
			input: &cloudinit.BaseUserData{
				PreRKE2Commands:  preRKE2Commands,
	...
	for _, tt := range tc {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			ignitionBytes, _, err := clc.Render(tt.input, &bootstrapv1.ContainerLinuxConfig{})
			if err != nil {
	...
	t.Run("validates input parameter", func(t *testing.T) {
		...
		if _, _, err := clc.Render(nil, &bootstrapv1.ContainerLinuxConfig{}); err == nil {
	...
	t.Run("accepts empty clc parameter", func(t *testing.T) {
		...
		if _, _, err := clc.Render(&cloudinit.BaseUserData{}, nil); err != nil {
	...
	t.Run("treats warnings as errors in strict mode", func(t *testing.T) {
		...
		if _, _, err := clc.Render(&cloudinit.BaseUserData{}, config); err == nil {
	...
	t.Run("returns warnings", func(t *testing.T) {
		...
		data, warnings, err := clc.Render(&cloudinit.BaseUserData{}, config)
		if err != nil {
