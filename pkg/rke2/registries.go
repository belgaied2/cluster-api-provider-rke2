...
func GenerateRegistries(rke2ConfigRegistry RKE2ConfigRegistry) (*Registry, []bootstrapv1.File, error) {
	...
	for configName, regConfig := range rke2ConfigRegistry.Registry.Configs {
		...
		if !ok {
			rke2ConfigRegistry.Logger.Error(err, "Auth Secret for the registry is missing entries! Possible entries are: (\"username\" AND \"password\") OR \"identity-token\" ")
			//TODO: Add the ACTUAL map keys to the log message? Need to concat the map keys in a comma separated list and add them to the log message attributes.
			return &Registry{}, []bootstrapv1.File{}, err
		}
